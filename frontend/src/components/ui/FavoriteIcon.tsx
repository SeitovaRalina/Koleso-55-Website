interface FavoriteIconProps {
  active?: boolean
  variant?: 'overlay' | 'solid'
  className?: string
}

export function FavoriteIcon({
  active = false,
  variant = 'overlay',
  className = 'h-7 w-7',
}: FavoriteIconProps) {
  const inactiveClasses =
    variant === 'overlay'
      ? 'fill-white/90 stroke-brand-deep'
      : 'fill-none stroke-brand-deep'

  const activeClasses = 'fill-brand-deep stroke-brand-deep'

  return (
    <svg
      viewBox='0 0 24 24'
      fill='none'
      xmlns='http://www.w3.org/2000/svg'
      className={className}
      aria-hidden
    >
      <path
        d='M7.5 4C4.4625 4 2 6.4625 2 9.5C2 15 8.5 20 12 21.163C15.5 20 22 15 22 9.5C22 6.4625 19.5375 4 16.5 4C14.64 4 12.995 4.9235 12 6.337C11.4928 5.61469 10.819 5.0252 10.0357 4.61841C9.25238 4.21162 8.38263 3.9995 7.5 4Z'
        className={`${active ? activeClasses : inactiveClasses} transition-colors`}
        strokeWidth={2}
        strokeLinecap='round'
        strokeLinejoin='round'
      />
    </svg>
  )
}
